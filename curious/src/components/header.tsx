...
export default function Header() {
  ...
  return (
    <header className="bg-white dark:bg-black text-black dark:text-white flex justify-between items-center w-full p-5 border-b-2 border-lightGrey dark:border-black pb-7 sm:px-4 px-2">
      ...
      {router.pathname === '/feed' || router.pathname === '/search' ? (
        <>
        ...
        <div className='flex flex-row gap-4'>
          <Link href="/search">
            <button className="bg-white text-black border-solid border-2 border-black rounded-lg py-1 px-4">Sign Up</button>
          </Link>
          <Link href="/search">
            <button className="bg-black text-white border-solid border-2 border-black rounded-lg py-1 px-4">Sign In</button>
          </Link>
        </div>
      )}
    </header>
